...
*The variable 'OCCODE' furthermore describes the study outcome after six month in moire granular way by providing 5 potential values: 1:dead, 2:dependent, 3:not recovered, 4:recovered, 8 or 9: missing status.*
...
*First, just include the variables from sction: "Randomisation data" (according to: http://datashare.is.ed.ac.uk/bitstream/handle/10283/128/IST_variables.pdf). This excludes all the information that were not collected before Randomisation at the beginning of the study. Afterwards, only the country code ("CNTRYNUM") is added as a variable to the train and test data set. Finally, the dummy variable "DOD" is kept for training and test data as a label.*
*All other varibles basically contained study results after 14 days or indicated or diagnose variables, which should be excluded.*
```{r eval=T, message=F}
...
*To remove individuals with unknown outcome after six month, the 'OCCODE' variable is analyzed and rows removed, where no outcome of the study was not docmented*
```{r eval=T, message=F}
...
```
*Furthermore, specific cases were discovered, where individuals' initial event was not a stroke, where causes noise within the training and test data. Therefore, these instances were removed, too.*
...
*Due to the low accuracies of the applied algoirthms (below 0.8), one can state that we could not build a conclusive model. Comparing the different algorithms, identifies the Decision Tree and the Logisitc Regression to perform slightly better. However, the error rate of all models outlines the lack of significance. *
...
```
*From this small Average Treatment Effect of Aspirin (1.3%) compared to 0.0% of Heparin, we can conclude that Aspirin is not significantally decreading the outcome of death or dependence at 6 month.*

- of the algorithms tested, which algorithms perform the best? Justify your statement.

*Considering the results of each classifier on the test set, which where presented above, we can conclude that the Logistic Regression as well as the Decision Treee perform better than the other two classifiers given a higher accuracy.*
